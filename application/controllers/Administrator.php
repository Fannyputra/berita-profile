...
class Administrator extends CI_Controller {
	...
	function edit_halamanbaru(){
		...
		if (isset($_POST['submit'])){
            ...
            if($_FILES["c"]["name"] == ""){
                ...
                $hasil['file_name'] = $foto_lama;

            } else if($_FILES["c"]["name"]) {
                $this->upload->do_upload('c');
                $hasil=$this->upload->data();
            }           
            if ($hasil['file_name']==''){
	...
	function tambah_kategoriberita(){
		...
		if (isset($_POST['submit'])){
            ...
            $config['file_name'] = $new_name;
            $config['upload_path'] = 'asset/foto_berita/';
            $config['allowed_types'] = 'gif|jpg|png|jpeg|swf|JPG|JPEG|PNG|SWF';
	...
	function edit_kategoriberita(){
		...
		if (isset($_POST['submit'])){
            ...
            $config['file_name'] = $new_name;
            $config['upload_path'] = 'asset/foto_berita/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf|PNG|SWF';
            // $config['max_size'] = '5000'; // kb
            $this->load->library('upload', $config);
            if($_FILES["d"]["name"] == ""){
                $foto_lama = $this->input->post('foto_lama');
    ...
    function tambah_iklanatas(){
        ...
        if (isset($_POST['submit'])){
            ...
            $config['allowed_types'] = 'gif|jpg|png|jpeg|swf|JPG|JPEG|PNG|GIF|SWF';
            // $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            $hasil=$this->upload->data();
	...
	function tambah_download(){
		...
		if (isset($_POST['submit'])){
            ...
            $config['allowed_types'] = 'gif|jpg|png|jpeg|swf|JPG|JPEG|PNG|GIF|SWF|zip|rar|pdf|doc|docx|ppt|pptx|xls|xlsx|txt';
