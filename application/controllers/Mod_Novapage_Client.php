...
class Mod_Novapage_Client extends CI_Controller {
	public function index(){          
        cek_session_akses('novapage-client',$this->session->id_session);  
        ...
        if(isset($_POST['set_client'])) {
            ...
            redirect($this->uri->segment(1).'/novapage-client');
    ...
    protected function view_index(){         
        $data['get_client']  = $this->model_app->view_ordering('tbl_novapage_client','nama','ASC') ; 
        $this->template->load('administrator/template','administrator/mod_novapage_client/view_index',$data);
    } 
    ...
    protected function save_data($id='',$data = array()) {
        ...
        // get data
        $get_data  = $this->model_app->view_where('tbl_novapage_client',array('id_client' => $id ))->row_array(); 
        ...
        if( !empty($data) && is_array($data)) { 
            if(!empty($get_data) && $get_data['id_client'] == $id ){  
                ...
                return $this->model_app->insert('tbl_novapage_client', $data);
            }
        } else {
            return null;
        }
    ...
    protected function upload_logo($param){
        ...
        $config['upload_path'] = 'asset/img_novapage/client/';
    ...
    protected function hapus_logo($nama_logo = '') {
        // jika ada logo hapus
        $logo_file = FCPATH .'asset/img_novapage/client/'.$nama_logo; 
