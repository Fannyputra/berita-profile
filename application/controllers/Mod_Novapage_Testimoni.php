...
class Mod_Novapage_Testimoni extends CI_Controller {
	public function index(){          
        cek_session_akses('novapage-testimoni',$this->session->id_session);  
        ...
        if(isset($_POST['set_testimoni'])) {
            ...
            redirect($this->uri->segment(1).'/novapage-testimoni');
    ...
    protected function view_index(){         
        $data['get_testimoni']  = $this->model_app->view_ordering('tbl_novapage_testimoni','nama','ASC') ; 
        $this->template->load('administrator/template','administrator/mod_novapage_testimoni/view_index',$data);
    } 
    ...
    protected function save_data($id='',$data = array()) {
        ...
        // get data
        $get_data  = $this->model_app->view_where('tbl_novapage_testimoni',array('id_testimoni' => $id ))->row_array(); 
        ...
        if( !empty($data) && is_array($data)) { 
            if(!empty($get_data) && $get_data['id_testimoni'] == $id ){  
                ...
                return $this->model_app->insert('tbl_novapage_testimoni', $data);
            }
        } else {
            return null;
        }
    ...
    protected function upload_photo($param){
        ...
        $config['upload_path'] = 'asset/img_novapage/testimoni/';
    ...
    protected function hapus_photo($nama_photo = '') {
        ...
        $photo_file = FCPATH .'asset/img_novapage/testimoni/'.$nama_photo; 
